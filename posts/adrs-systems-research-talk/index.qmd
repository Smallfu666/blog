...
For example, if two adjacent rows both start with the same instruction and share fields like product category or review template, the LLM may reuse more prefix KV cache. The goal is not to change the query result. The goal is to arrange the table so that the prompts become cheaper to execute.
...
For a toy workload, imagine three transactions:

- `T1` writes hot key `A`
- `T2` touches only `B`
- `T3` writes hot key `A`

If the scheduler places `T1` and `T3` back-to-back, the hot key is hit twice in a row. A better order inserts `T2` between them so the conflict pressure is lower.

SMF is a greedy policy: at each step it samples candidate transactions and appends the one that increases makespan the least.
...
## Workflow note: AI-assisted paper reading
